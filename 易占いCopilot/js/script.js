...
startButton.addEventListener('click', () => {
    ...
    for (let i = 0; i < 6; i++) {
        setTimeout(() => {
            ...
            coins.push(isYang ? 1 : 0);
            const coinElement = document.createElement('div');
            coinElement.className = `coin ${isYang ? 'white' : 'black'}`;
            coinDisplay.appendChild(coinElement);

            if (coins.length === 6) {
                const hexagramBinary = coins.join('');
                const hexagramIndex = parseInt(hexagramBinary, 2);

                if (hexagrams[hexagramIndex]) {
                    const hexagram = hexagrams[hexagramIndex];
                    result.innerHTML = `
                        <div>卦: ${hexagram.name} </div>
                        <div>読み仮名: ${hexagram.reading}</div>
                        <div>意味: ${hexagram.meaning}</div>
                    `;
                } else {
                    error.textContent = 'エラー。もう一度占ってください';
                }
            }
        }, i * 1000);
    }
